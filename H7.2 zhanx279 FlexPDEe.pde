...
BOUNDARIES
surface 'bottom' ! z = 0
value(u)=0 !change to load=0 when needed 
value(v)=0 !change to load=0 when needed 
value(w)=0 !change to load=0 when needed 
surface 'top' !z = Lz
load(u)=s_applied
load(v)=s_applied/2
load(w)=0
...
grid(x, y, z)	
grid(x+u, y+v, z+w)
 	contour(u) on surface z=0 painted
